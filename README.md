...
### Possible Otimizations to the code
  ...
  Although I used to do the task React with Hooks, React Spring and other libraries and complements to make a web site more performatic, there is still some otimizations which could be done. We live in world which more than half of population still just have 2G internet, with that we should be really worried about our web site performance. We must be worried about our bundle size, images and every package our website consumes in the customer browser. So problably I would try Angular IVY, even though Angular are still testing that to the next versions of Angular, I have heard a lot of how it will change the Web because mostly because of how it impacts of perfomance of web sites. 

  I would do improve a lot acessibility questions and responsive design. Furthermore I would test my components  with `Jest`, `Jasmine` and `react-test-library`. This way I would make sure of each behavior of my components and my form. This would guarantee its funcionality.

### Which things could be done better, than you’ve done it?

  In my opiniom, using React, Sass, React Spring, Redux and many other tools improved a lot the quality of the code, performance, design and engineering of the app. If I had just using Vanilla Js ou Jquery problably I would have taken more time and end up in spaghetti code somehow. This would make the code hard to undestand and to maintain. Using this news frameworks most of the time bring some benefits, because thay solve problems which older versions of HTML, CSS and JS have had.

  I hade some problemas with the API endpoint given. It has been returning 400 error related to CORS problem. This way I could not show to the user the `paymentId`. I should save the `paymentId`, and show it to the user. Event though I could not retrieve that because of the mentioned erros, I saved it in tha Local Dataset of the browser. I understood as a fronten task I should save in a Local Storage dataset. So as a improvement I would have saved that in a local database as weel. I would create am API using some PHP frameworks, such as, Laravel YII2 and so on. I would create a post method, connect to MYsql database, and this method would be responsible for saved all user data, including the `paymentId`. These frameworks usually create almost everything foi you based in MVC pattern. So, my frontend would a call the endpoint responsible to save in my database, just after the Wunder API promise been resolved. 
  And then my controler would send the data to a simple service, which would connect to the database using a model. So as you can see, it is not hard to have it done.    
